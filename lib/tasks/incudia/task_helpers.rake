...
namespace :incudia do
  ...
  # Raises Incudia::TaskAbortedByUserError if the user chose *not* to continue
  def ask_to_continue
    answer = prompt("Do you want to continue (yes/no)? ".blue, %w{yes no})
    raise Incudia::TaskAbortedByUserError unless answer == "yes"
  end
  ...
  def run(command)
    output, _ = Incudia::Popen.popen(command)
    output
  rescue Errno::ENOENT
  ...
  def warn_user_is_not_incudia
    unless @warned_user_not_incudia
      incudia_user = Incudia.config.incudia.user
      current_user = run(%W(whoami)).chomp
      unless current_user == incudia_user
        puts "#{Colored.color(:black)+Colored.color(:on_yellow)} Warning #{Colored.extra(:clear)}"
        puts "  You are running as user #{current_user.magenta}, we hope you know what you are doing."
        puts "  Things may work\/fail for the wrong reasons."
        puts "  For correct results you should run this as user #{incudia_user.magenta}."
        puts ""
      end
      @warned_user_not_incudia = true
    end
  end
end
